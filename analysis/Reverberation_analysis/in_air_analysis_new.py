...
import utils_analysis_new as utils
...
class QCObject:
    ...
    def readDICOMtag(self, key, imslice=0):         # slice=2 is image 3
        value = utils.readDICOMtag(key, self.dcmInfile, imslice)
        return value
...
class analysis(QCObject):
    def __init__(self, dcmInfile, pixelData):
        ...
        self.params = {
            ...
            # [158, 68, 865, 143], # in px to be used to restrict initial search of reverb pattern; three full rings (peak-to-peak); approx 3*25
    ...
    def save_annotated_image(self, fname, what='overview', xtra={}):
        ...
        if what == 'overview':
            ...
            im = utils.toimage(work, low=1, pal=pal)
    ...
    def isolate_reverberations(self):
        ...
        if self.verbose:
            ...
            utils.toimage(reverb_image).save('reverbimage.jpg')
    ...
    def find_curve(self, rev_mask):
        ...
        if None in [curve_radii, curve_xyr, curve_angles]:
            ...
            if circle_fitfrac < 1 and circle_fitfrac > 0:
                ...
                cf = utils.CircleFit(circ_xy[int(fff * len(circ_xy)):])
            else:
                cf = utils.CircleFit(circ_xy)  # seems best for GE
    ...
    def straighten_curve(self, rev_mask):
        ...
        if self.verbose:
            utils.toimage(rect_image).save('uncurvedreverb.jpg')
